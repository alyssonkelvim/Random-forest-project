package project.src.java.approaches.gpu.table.count;
...
public class TableRestrictCountGPUGenerator {
	public static void execute(List<Tree> trees, String dataset) {
        ...
        sourceCode += generateMainFunction(trees, Parser.featuresNames.size(), Parser.samplesQuantity);
        sourceCode = sourceCode.replaceAll("%TABLE_SIZE%", tableSize+"");

        System.out.println("Writing File...");
        FileBuilder.execute(sourceCode, "gpu/table/restrict/rf_with_table_count.cu");
    }
	...
	private static String generateThreadAndTrees(int featuresQuantity, int classesQuantity, List<Tree> trees) {
	    ...
	    String code = "__global__ void RF_with_table(const int* __restrict__ table, const float* __restrict__ thresholds,"+features+", int *P, const int N){\n";
	...
	private static String generateThresholdVector(List<Tree> trees) {
		String code = "float thresholdsData[] = \n\t{ %VALUES% };\n";
    ...
    private static String generateMainFunction(List<Tree> trees, int featureQuantity, int samplesQuantity) {
        ...
        "    size_t nTableBytes = %TABLE_SIZE% * sizeof(int);" +
		"    " + generateTreeVector(trees) +
		"    " + generateThresholdVector(trees) +
        "\n" +
        ...
        "    float %_DECLARING_GLOBAL_MEMORY_%,  * thresholds;\n" +
        "    int * d_P, * table;\n" +
        ...
        "    CHECK(cudaMemcpy(thresholds, thresholdsData, nTableBytes, cudaMemcpyHostToDevice));" +
        ...
        "    RF_with_table << < grid, block >>> (table, thresholds, %_VARIABLES_%, d_P, nElem);\n" +
